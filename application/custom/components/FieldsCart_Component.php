<?php
class FieldsCart_Component extends Component {
	...
	private function get_row_object($fieldMetadata){
		$render ='			<tr class="' . $this -> variable . '_item">
                            	...
                            	<td><input type="number" class="form-control" name="' . $this -> variable . '_field_count[]" value="' . $fieldMetadata -> {$this -> variable . "_field_count"} . '"></td>
                            	<td><input type="number" class="form-control" name="' . $this -> variable . '_field_price[]" value="' . $fieldMetadata -> {$this -> variable . "_field_price"} . '"></td>
                            	<td><input type="text" class="form-control" name="' . $this -> variable . '_field_note[]" value="' . $fieldMetadata -> {$this -> variable . "_field_note"}. '"></td>
	...
	private function get_empty_row_object(){
		$render ='			<tr class="' . $this -> variable . '_empty" style="display:none;">
                            	...
                            	<td><input type="number" class="form-control" name="' . $this -> variable . '_field_price[]" value=""></td>
                            	<td><input type="text" class="form-control" name="' . $this -> variable . '_field_note[]" value=""></td>
                                <td><button class="delete btn btn-danger glyphicon glyphicon-trash" title="刪除欄位" type="button"></button></td>
                            </tr>';
		return $render;
	}
	...
	public function set_value( $value ){
		...
		$field_note = $this -> variable . "_field_note";
		...
		for($i=0;$i<$count-1;$i++){
			...
			$fields[] = array(
				...
				$field_note => $this -> controller -> module_io -> {$field_note}[$i]
			);
		}
