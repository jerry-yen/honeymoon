<?php
class FieldsImage_Component extends Component {
	...
	private function get_row_object($fieldMetadata){
        ...
        if($fieldMetadata -> {$this -> variable . "_field_image"} == ""){
			...
			$render .=           	'<td><img src="' . $this -> config["machine_relative_full_upload_path"] . $fieldMetadata -> {$this -> variable . "_field_image"} . '" height="100"><input type="hidden" name="' . $this -> variable . '_field_image[]" value="' . $fieldMetadata -> {$this -> variable . "_field_image"} . '"></td>';
		}
