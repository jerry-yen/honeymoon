<?php
class Fix_Item_Controller extends AdminController {
	...
	public function save() {
		...
		if($valid_success){
			...
			if(gettype($this -> item -> updateTime) == 'object'){
				$this -> item -> updateTime -> set_value(date('Y-m-d H:i:s'));
			}
			else{
	...
	public function fix_password(){
		...
		if($valid_success){
			...
			if(gettype($update_object -> password) == 'object'){
				$update_object -> password = $update_object -> password -> get_value();
			}
			//var_dump($update_object -> password);
