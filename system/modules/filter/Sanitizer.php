...
class HTML_Sanitizer {
	...
	public function resetAll() {
		...
		$this -> _allowHTMLNoneEscape = false;

		$this -> _allowedTags = '<a><br><b><h1><h2><h3><h4><h5><h6>' . '<img><li><ol><p><strong><table><tr><td><th><u><ul><thead>' . '<tbody><tfoot><em><dd><dt><dl><span><div><del><add><i><hr>' . '<pre><br><blockquote><address><code><caption><abbr><acronym>' . '<cite><dfn><q><ins><sup><sub><kbd><samp><var><tt><small><big>';
	...
	/**
	 * Helper to allow all HTML related tags None Escape
	 * @access public
	 */
	public function allowHTMLNoneEscape() {
		$this -> _allowHTMLNoneEscape = true;
	}
	...
	private function removeDOMEvents($str) {
		...
		$HTML_Sanitizer_stripAttrib = '(onclick|ondblclick|onmousedown|' . 'onmouseup|onmouseover|onmousemove|onmouseout|onkeypress|onkeydown|' . 'onkeyup|onfocus|onblur|onabort|onerror|onload)';

		if(is_array($str)){
			foreach($str as $key => $sstr){
				$str[$key] = stripslashes(preg_replace("/$HTML_Sanitizer_stripAttrib/i", 'forbidden', $str[$key]));
			}
		}
		else{
	...
	private function removeEvilTags($str) {
		...
		if(is_array($str)){
			foreach($str as $key => $sstr){
				$str[$key] = strip_tags($str[$key], $allowedTags);
			}
		}
		else{
	...
	public function sanitize($html) {
		...
		$html = $this -> removeDOMEvents($html);

		$html = $this -> removeJavascriptURL($html);

		return ($this -> _allowHTMLNoneEscape) ? $html : htmlspecialchars($html);
	}
